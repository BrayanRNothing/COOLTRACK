...
export default function ClientProfilePage() {
  ...
  const { getCliente, getClimasByCliente } = useWorkData()
    ...
    <section className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6 items-stretch">
          ...
          <article className="rounded-2xl border border-base-300 bg-base-100 p-6 shadow-sm overflow-hidden h-full flex flex-col">
            ...
            <div className="space-y-5 flex-grow">
              <div className="grid grid-cols-1 gap-4">
                <div className="bg-base-200/40 p-4 rounded-2xl border border-base-300/30">
                  <span className="text-[10px] font-bold uppercase opacity-40 block mb-1">Ubicación / Ciudad</span>
                  <p className="font-semibold text-sm text-base-content">{cliente.ciudad || 'No especificada'}</p>
                </div>
                <div className="bg-base-200/40 p-4 rounded-2xl border border-base-300/30">
                  <span className="text-[10px] font-bold uppercase opacity-40 block mb-1">Teléfono de contacto</span>
                  <p className="font-semibold text-sm text-base-content">{cliente.telefono || 'Sin teléfono'}</p>
                </div>
                <div className="bg-base-200/40 p-4 rounded-2xl border border-base-300/30">
                  <span className="text-[10px] font-bold uppercase opacity-40 block mb-1">Correo Electrónico</span>
                  <p className="font-semibold text-sm break-all text-base-content">{cliente.email || 'Sin correo'}</p>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-2 h-full">
          <div className="rounded-2xl border border-base-300 bg-base-100 p-6 shadow-sm h-full flex flex-col">
            ...
            <div className="grid gap-3 flex-grow overflow-auto">
              {recientes.length === 0 ? (
                ...
                recientes.map(clima => (
                  <div 
                    key={clima.id} 
                    className="rounded-2xl border border-base-300 bg-base-100 p-4 hover:border-primary/40 hover:bg-primary/5 hover:translate-x-1 transition-all cursor-pointer group"
